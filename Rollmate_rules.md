...
- the player must wait until the dice gives a piece type that can **legally resolve the check**
- or the dice gives the **king**, and the king has a legal move that resolves the check

### 8.2 What counts as a valid response to check

A move is legal in a check situation only if it removes the check condition according to traditional chess rules.This includes any move that:
...
- that roll does not grant a playable move
- the player remains in the check state
- the player must continue waiting for a future dice result that can resolve the check
...
4. If no, the player must keep waiting for a future dice result that can resolve the check.
...
## 14. One-Line Rule Statement

**The dice determines the eligible piece type for the turn, but the player may move only if at least one legal move exists for that piece type; when in check, only moves that legally resolve the check are valid, and the game begins with White’s turn.**
